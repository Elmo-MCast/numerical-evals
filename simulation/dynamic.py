...
class Dynamic:
    def __init__(self, data, num_tenants, num_events, algorithm, num_bitmaps,
        ...
        self.switch_event_types_to_update_count = self.dynamic['switch_event_types_to_update_count']
        ...
        if self.debug:
            print(pd.Series(self.switch_event_types_to_update_count['virtual']['J']).describe())
            print(pd.Series(self.switch_event_types_to_update_count['virtual']['L']).describe())
            print(pd.Series(self.switch_event_types_to_update_count['leaf']['J']).describe())
            print(pd.Series(self.switch_event_types_to_update_count['leaf']['L']).describe())
    ...
    def _run(self):
        for t in bar_range(self.num_tenants, desc='dynamic:%s' % self.algorithm):
            ...
            groups_map = tenant_maps['groups_map']
            for g in range(group_count):
                Event(self.switch_event_types_to_update_count, vms_map, self.algorithm, self.leafs_to_rules_count_map,
                      self.num_bitmaps, self.num_leafs_per_bitmap, self.redundancy_per_bitmap, self.num_rules_per_leaf,
                      self.probability, groups_map[g], self.min_group_size, vm_count, self.num_hosts_per_leaf)
