...
class Data:
    ...
    #     hist = pd.cut(self.leafs_for_all_tenants, [i for i in range(self.data.placement.num_bitmaps + 1)],
    #                   labels=[i for i in range(self.data.placement.num_bitmaps)]).value_counts()
    #     percentage_hist = hist / self.data.tenants.tenant_group_count_map.sum() * 100
    #     self.percentage_hist_of_groups_covered_with_varying_bitmaps = percentage_hist.sort_index()
    #
    # def _get_rules_for_all_leafs(self):
    #     self.rules_for_all_leafs = [0] * self.data.network.num_leafs
    #
    #     for t in range(self.data.tenants.num_tenants):
    #         for g in range(self.data.tenants.tenant_group_count_map[t]):
    #             if self.data.placement.tenant_groups_to_leaf_count[t][g] > self.data.placement.num_bitmaps:
    #                 for l in self.data.placement.tenant_groups_to_leafs_map[t][g]:
    ...
    #     for t in range(self.data.tenants.num_tenants):
    #         for g in range(self.data.tenants.tenant_group_count_map[t]):
    #             if self.data.optimization.tenant_groups_to_redundancy_map[t][g] is not None:
    #                 self.redundancy_for_all_tenants = self.redundancy_for_all_tenants.append(
    #                     pd.Series(self.data.optimization.tenant_groups_to_redundancy_map[t][g]), ignore_index=True)
    #
    ...
    #     for t in range(self.data.tenants.num_tenants):
    #         for g in range(self.data.tenants.tenant_group_count_map[t]):
    #             if self.data.optimization.tenant_groups_to_min_bitmap_count[t][g] is not None:
    #                 self.min_bitmaps_for_all_tenants = self.min_bitmaps_for_all_tenants.append(
    #                     pd.Series(self.data.optimization.tenant_groups_to_min_bitmap_count[t][g]), ignore_index=True)
