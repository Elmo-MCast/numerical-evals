...
class Tenants:
    def __init__(self, data, max_vms_per_host=20, num_tenants=3000, min_vms=10, max_vms=5000, vm_dist='expon',
                 num_groups=100000, min_group_size=5, group_size_dist='uniform', num_jobs=4, debug=False):
        self.data = data
    ...
    def _get_tenant_to_vm_count_map(self):
        if self.vm_dist == 'expon':
            ...
            vm_counts[non_outliers] = (((np.random.exponential(scale=1 / 4.06, size=len(non_outliers[0])) / 10)
