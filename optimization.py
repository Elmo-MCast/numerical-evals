...
def dp_algorithm(num_hosts_per_leaf, leaf_bitmasks, num_bitmasks):
    return ([BitArray(num_hosts_per_leaf)] * num_bitmasks,
            {i: i for i in range(num_bitmasks)},
            10)
...
class Optimization:
    def __init__(self, network, tenants, placement):
        ...
        self.tenant_groups_to_bitmasks_map = None
        self.tenant_groups_leafs_to_bitmask_id_map = None
        self.tenant_groups_to_redundancy_map = None

        self._optimize()

    def _optimize(self):
        self.tenant_groups_to_bitmasks_map = [None] * self.tenants.num_tenants
        self.tenant_groups_leafs_to_bitmask_id_map = [None] * self.tenants.num_tenants
        self.tenant_groups_to_redundancy_map = [None] * self.tenants.num_tenants

        for t in range(self.tenants.num_tenants):
            _groups_to_bitmasks_map = [None] * self.tenants.tenant_group_count_map[t]
            _groups_leafs_to_bitmask_id_map = [None] * self.tenants.tenant_group_count_map[t]
            _groups_to_redundancy_map = [None] * self.tenants.tenant_group_count_map[t]

            for g in range(self.tenants.tenant_group_count_map[t]):
                if self.placement.tenant_groups_to_leaf_count[t][g] > self.placement.num_bitmasks:
                    (_groups_to_bitmasks_map[g], _groups_leafs_to_bitmask_id_map[g], _groups_to_redundancy_map[g]) = \
                        dp_algorithm(
                            self.network.num_hosts_per_leaf,
                            self.placement.tenant_groups_leafs_to_bitmask_map[t][g],
                            self.placement.num_bitmasks
                        )

            self.tenant_groups_to_bitmasks_map[t] = _groups_to_bitmasks_map
            self.tenant_groups_leafs_to_bitmask_id_map[t] = _groups_leafs_to_bitmask_id_map
